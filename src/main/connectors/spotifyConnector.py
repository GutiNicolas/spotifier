...
SPOTIFY_ADD_SONG_TO_PLAYLIST = "https://api.spotify.com/v1/playlists/{}/tracks?uris={}"
SPOTIFY_QUERY_FOR_PLAYLIST_ITEMS = "https://api.spotify.com/v1/playlists/{}/tracks"

DEFAULT_HEADERS = {"Content-Type": "application/json",
                   "Authorization": "Bearer {}".format(spotify_properties.client_secret)}
...
class spotify_connector:
    ...
    def get_spotify_auth_token(self):
        requests.get(
            "https://accounts.spotify.com/authorize?response_type=code&client_id={}&scope=user-read-private%20user-read-email%20playlist-modify-private%20playlist-modify-public")

    def get_spotify_uri(self):
        pass

    def add_song_to_playlist(self, playlist_id, song_ids):
        logger.info("Adding Song with ids [{}] to playlist [{}]".format(song_ids, playlist_id))
        r = requests.post(SPOTIFY_ADD_SONG_TO_PLAYLIST.format(playlist_id, ",".join(song_ids)),
                          headers=DEFAULT_HEADERS)
        print("STATUS: {}  data: {}".format(r.status_code, r.json()))

    def create_spotify_playlist(self, name, description, public):
        body = json.dumps({"name": name, "description": "{}. created using github.com/gutiNicolas/spotifier".format(description), "public": public})
        ...
        r_json = r.json()
        print(r_json)
        logger.info("New playlist ID [{}]".format(r_json["id"]))
        return r_json["id"]
    ...
    def search_from_playlist(self, playlist):
        playlist_id = playlist.split("playlist:")[1]
        logger.info("[SearchFromPlaylist] Using {} as Playlist ID".format(playlist_id))
        if StringUtils.is_empty(playlist_id):
            logger.warn("[SearchFromPlaylist] Spotify Playlist ID not found on playlist_raw {}".format(playlist))
            return []

        logger.info("[SearchFromPlaylist] Requesting data for Spotify Playlist with ID {}".format(playlist_id))

        r = requests.get(SPOTIFY_QUERY_FOR_PLAYLIST_ITEMS.format(playlist_id), headers=DEFAULT_HEADERS)
        r_json = r.json()
        print(r_json)

        found_songs = []
        if r_json is not None and CollectionUtils.none_empty(r_json["items"]):
            for song in r_json["items"]:
                song_name = song["track"]["name"]
                artist_name = song["track"]["artists"][0]["name"]
                found_songs.append((artist_name, song_name))
